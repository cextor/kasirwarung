...
import { formatRupiah, generateEscPosBytes, sendToPrinter } from '../utils/bluetoothPrinter';
...
export default function SalesHistory({
  ...
  const handleReprint = async (sale: Sale) => {
    if (!activePrinter || !activePrinter.characteristic) {
      alert('Hubungkan printer Bluetooth Anda terlebih dahulu melalui panel Kasir atau Koneksi Printer.');
      return;
    }
    ...
    try {
      ...
      console.error(err);
      alert('Gagal mencetak ulang struk. Silakan hubungkan ulang printer.');
    } finally {
      setIsPrinting(null);
    }
  };
  ...
  const handleDeleteSale = (sale: Sale) => {
    if (confirm(`Apakah Anda yakin ingin membatalkan transaksi ${sale.invoiceNumber}?\n\nTindakan ini akan mengembalikan stok barang ke semula dan menghapus rekap penjualan ini.`)) {
