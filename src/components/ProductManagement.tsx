...
import { Plus, Trash2, Edit2, ShoppingBag, Search, Filter, RefreshCcw, Tag } from 'lucide-react';
...
export default function ProductManagement({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (wholesaleMinQty === '' || wholesaleMinQty <= 0) return setError('Jumlah minimal grosir minimal 1.');
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6" id="product-mgmt-container">
      {/* Form Card */}
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm h-fit xl:sticky xl:top-6" id="product-form-card">
        ...
        <form onSubmit={handleSubmit} className="space-y-4" id="product-form">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-1">
                Min. Qty Grosir <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-800"
          ...
          {error && <p className="text-xs text-red-500" id="product-error">{error}</p>}

          <div className="flex gap-2 pt-2">
            <button
              type="submit"
              className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2.5 px-4 rounded-xl shadow-md shadow-indigo-100 transition-all text-sm cursor-pointer"
              id="product-submit-btn"
            >
              {editingProduct ? 'Simpan Perubahan' : 'Tambah Produk'}
            </button>
            {(editingProduct || name || categoryId || priceRetail) && (
              <button
                type="button"
                onClick={resetForm}
      ...
      {/* Product List Panel */}
      <div className="xl:col-span-2 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col h-full" id="product-list-panel">
        {/* Header with Search and Filters */}
        <div className="border-b border-slate-100 pb-4 mb-4" id="product-filter-header">
          <div className="flex flex-col md:flex-row md:items-center gap-4 justify-between">
            <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2">
              <span>Daftar Katalog Produk</span>
              <span className="text-xs bg-slate-100 text-slate-600 font-medium px-2.5 py-0.5 rounded-full" id="product-total-badge">
          ...
          {/* Filter Bar */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4" id="filters-grid">
            {/* Search Input */}
            <div className="relative">
          ...
          <div className="space-y-4" id="products-lists-container">
            ...
            <div className="md:hidden space-y-3.5" id="products-mobile-list">
              {filteredProducts.map((product) => {
                ...
                return (
                  <div key={product.id} className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm space-y-3 relative" id={`product-card-${product.id}`}>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50/60 p-3 rounded-xl border border-slate-100/50">
                      ...
                      <div>
                        ...
                        <span className="text-[9px] text-slate-400 block font-medium">Min. {product.wholesaleMinQty} {product.unit}</span>
                      </div>
                    </div>
                    ...
                    return (
                      <tr key={product.id} className="hover:bg-slate-50 transition-colors" id={`product-row-${product.id}`}>
                        ...
                        <td className="py-3.5 px-4 text-right text-slate-600">
                          ...
                          <div className="text-[10px] text-slate-400 font-medium">min {product.wholesaleMinQty} {product.unit}</div>
                        </td>
