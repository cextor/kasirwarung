# PANDUAN INSTALASI & MENJALANKAN KASIR WARUNG SECARA OFFLINE (XAMPP - DRIVE D)

Panduan ini ditujukan bagi pengguna awam untuk menjalankan aplikasi **Kasir Warung** secara offline menggunakan XAMPP yang terpasang di **Drive D:** pada OS Windows 10 Compact. Panduan dilengkapi cara membuat shortcut sekali klik langsung dari Desktop.
...
1. **Salin Folder Aplikasi ke htdocs**:
   * Pindahkan atau salin folder **`kasirwarung`** Anda ke dalam direktori XAMPP htdocs di Drive D:
   * Jalur lengkap folder aplikasi Anda harus berupa:
     `D:\xampp\htdocs\kasirwarung`
...
2. **Membuat Database di phpMyAdmin**:
   ...
   * Pilih database `kasirwarung` yang baru dibuat tersebut, masuk ke tab **Import / Impor**, pilih file **`schema.sql`** yang berada di dalam folder `D:\xampp\htdocs\kasirwarung\backend\schema.sql`, lalu klik **Import / Kirim** di bagian bawah.
   * *(Opsional)* Jika ingin menambahkan data contoh (seperti kategori mie, sembako, minuman, snack, dan beberapa barang demo), impor juga file **`seed.sql`** yang berada di `D:\xampp\htdocs\kasirwarung\backend\seed.sql`.

3. **Konfigurasi File Environment (.env)**:
   * Masuk ke folder `D:\xampp\htdocs\kasirwarung\backend`
...
1. Buka folder **`D:\xampp\htdocs\kasirwarung`**.
...
3. Sebuah jendela CMD akan muncul selama 3 detik untuk secara otomatis mendeteksi dan menyalakan server Apache & database MySQL di latar belakang.
